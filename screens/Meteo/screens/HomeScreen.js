...
export default function HomeScreen() {
  ...
  const fetchMyWeatherData = async () => {
    ...
    setCurrentAdress(`${currentLocation.coords.latitude},${currentLocation.coords.longitude}`)
    let myCity = await getData('city');
    let cityName = `${currentLocation.coords.latitude},${currentLocation.coords.longitude}`;
    if (myCity) {
      cityName = myCity;
    }
    fetchWeatherForecast({
        ...
        loading ? (
          ...
          <SafeAreaView className="flex flex-1">
            ...
            <View className="mb-2 space-y-3">
              <View className="flex-row items-center justify-between  mx-5 space-x-2">
                <View className="flex-row items-center mx-5 space-x-2">
                <CalendarDaysIcon size="22" color="white" />
                <Text className="text-white text-base">{t("Daily forecast")}  </Text>
                </View>
                <TouchableOpacity onPress={() => handleLocation({name:currentAdress})}>
                <Icon
